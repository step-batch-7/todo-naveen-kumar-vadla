...
const createTodo = (req, res) => {
  ...
  const [lastTodo] = todoList.todos;
  const todoId = getNewId(lastTodo);
  todoList.todos.unshift({ id: todoId, title, tasks: [] });
  serveTasks(req, res);
};

const removeTodo = (req, res) => {
  const { todoId } = JSON.parse(req.body);
  const todoIndex = getIndex(todoList.todos, todoId);
  todoList.todos.splice(todoIndex, NUMBER_ONE);
  serveTasks(req, res);
};
...
const addTask = (req, res) => {
  ...
  const todo = getTodo(todoId);
  const lastTask = todo.tasks[todo.tasks.length - NUMBER_ONE];
  const taskId = getNewId(lastTask);
  todo.tasks.push({ id: taskId, work, isCompleted: false });
  serveTasks(req, res);
};
