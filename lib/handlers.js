...
const TODO_DATA_FOLDER = `${__dirname}/../data/todoLists.json`;
...
const loadTasks = () => {
  if (isExistingFile(TODO_DATA_FOLDER)) {
    return JSON.parse(readFromFile(TODO_DATA_FOLDER) || '[]');
  }
  return [];
};
...
const serveTasks = (req, res) => {
  writeIntoFile(TODO_DATA_FOLDER, JSON.stringify(todoLists));
